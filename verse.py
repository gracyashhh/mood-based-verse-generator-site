...
verse=None
@app.before_request
def prepare_sentiment_verse(verse_path):
    global verse
    verse=pd.read_csv(verse_path)
    sid = SentimentIntensityAnalyzer()

    all_compounds=[]
    for sentence in verse['verse']:
        ss=sid.polarity_scores(sentence)
        for k in sorted(ss):
            if k=='compound':
                all_compounds.append(ss[k])
    verse['sentiment score']=all_compounds
    verse=verse.sort_values('sentiment score')
    verse['index']=[i for i in range(0,len(verse))]
    return verse
verse=prepare_sentiment_verse(r'C:\Users\Avita\Desktop\verses.csv')

max_index_value=np.max(verse['index'].values)

def gimme_a_verse(direction=None,current_index=None):
